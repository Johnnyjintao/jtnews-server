...
class UserController {
    ...
    static async create(ctx) {
        ...
        if (user.username && user.password) {
            ...
            if (existUser) {
                ...
                ctx.body = statusCode.SUCCESS_200(-1,'用户已经存在')
            } else {
                ...
                ctx.response.status = 200;
                ctx.body = statusCode.SUCCESS_200(0,'创建用户成功', token)
            }
        } else {
    ...
    static async getUserInfo(ctx) {
        ...
        if (token) {
            ...
            try {
                ...
                ctx.response.status = 200;
                ctx.body = statusCode.SUCCESS_200(0,'查询成功', user)
            } catch (err) {

                ctx.response.status = 200;
                ctx.body = statusCode.SUCCESS_200(0,'查询失败，authorization error!',err)
            }
        }
    }
    ...
    static async delete(ctx) {
        ...
        if (id && !isNaN(id)) {
            ...
            ctx.response.status = 200;
            ctx.body = statusCode.SUCCESS_200(0,'删除用户成功')
        } else {

            ctx.response.status = 200;
            ctx.body = statusCode.SUCCESS_200(-1,'用户ID必须传')
        }
    }
    ...
    static async login(ctx) {
        ...
        if (user) {
            ...
            if (bcrypt.compareSync(data.password, user.password)) {
                ...
                ctx.response.status = 200;
                ctx.body = statusCode.SUCCESS_200(-1,'用户名或密码错误');
            }
        } else {

            ctx.response.status = 200;
            ctx.body = statusCode.SUCCESS_200(-1,'用户不存在');
        }
    }
